...
def test_paragraph_on_one_line(simple, pdf):
    ...
    p = PlacedParagraphContent(simple, defined, pdf)
...
def test_styled_paragraph_on_one_line(styled, pdf):
    defined = Rect.make(left=0, top=0, width=150, height=40)
    p = PlacedParagraphContent(styled, defined, pdf)
    assert p.bad_breaks == 0
    assert p.ok_breaks == 0
...
def test_rectangle(pdf):
    r = PlacedRectContent(Rect.make(left=10, top=10, right=100, bottom=100), Style(), PDF.BOTH, pdf)
...
def test_group_bad_fits(simple, styled, pdf):
    simple_placed = PlacedParagraphContent(simple, Rect.make(left=0, top=40, width=80, height=40), pdf)
    styled_placed = PlacedParagraphContent(styled, Rect.make(left=80, top=5, width=24, height=40), pdf)

    gp = PlacedGroupContent([simple_placed, styled_placed], Rect.make(left=0, top=0, right=200, bottom=100))
...
def test_group_with_space_horizontal(simple, styled, pdf):
    a = PlacedParagraphContent(simple, Rect.make(left=10, top=40, right=200, height=40), pdf)
    b = PlacedParagraphContent(styled, Rect.make(left=200, top=5, right=350, height=40), pdf)

    gp = PlacedGroupContent([a, b], Rect.make(left=0, top=0, right=350, bottom=500))
...
def test_group_with_space_vertical(simple, styled, pdf):
    a = PlacedParagraphContent(simple, Rect.make(left=10, top=20, right=200, height=40), pdf)
    b = PlacedParagraphContent(styled, Rect.make(left=40, top=50, right=200, height=40), pdf)

    gp = PlacedGroupContent([a, b], Rect.make(left=0, top=0, right=200, bottom=100))
...
def test_group_with_space_vertical_second(simple, pdf):
    a = PlacedParagraphContent(simple, Rect.make(left=10, top=20, right=200, height=40), pdf)
    b = PlacedParagraphContent(Paragraph("a very long piece of text that will just fit"),
...
def test_table_in_plenty_of_space(simple, styled, pdf):
    ...
    p = PlacedTableContent(table, Rect.make(left=10, top=10, width=400, height=100), pdf)
...
def test_table_with_one_wraps(simple, styled, pdf):
    ...
    p = PlacedTableContent(table, Rect.make(left=10, top=10, width=280, height=100), pdf)
...
def test_table_with_several_wraps(simple, styled, pdf):
    ...
    p = PlacedTableContent(table, Rect.make(left=10, top=10, width=180, height=100), pdf)
    assert p.bad_breaks == 0
    assert p.ok_breaks == 3
...
def test_table_with_terrible_wraps(simple, styled, pdf):
    ...
    p = PlacedTableContent(table, Rect.make(left=10, top=10, width=40, height=100), pdf)
