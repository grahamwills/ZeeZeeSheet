...
def test_rectangle(pdf):
    r = PlacedRectContent(Rect(left=10, top=10, right=100, bottom=100), Style(), PDF.BOTH, pdf)
...
def test_group_bad_fits(simple, styled, pdf):
    simple_placed = PlacedParagraphContent(simple, Rect(left=0, top=40, width=80, height=40), pdf)
    styled_placed = PlacedParagraphContent(styled, Rect(left=80, top=5, width=24, height=40), pdf)
...
def test_group_with_space_horizontal(simple, styled, pdf):
    a = PlacedParagraphContent(simple, Rect(left=10, top=40, right=200, height=40), pdf)
    b = PlacedParagraphContent(styled, Rect(left=200, top=5, right=350, height=40), pdf)
...
def test_group_with_space_vertical(simple, styled, pdf):
    a = PlacedParagraphContent(simple, Rect(left=10, top=20, right=200, height=40), pdf)
    b = PlacedParagraphContent(styled, Rect(left=40, top=50, right=200, height=40), pdf)
...
def test_table_in_plenty_of_space(simple, styled, pdf):
    ...
    p = PlacedTableContent(table, Rect(left=10, top=10, width=400, height=100), pdf)
...
def test_table_with_one_wraps(simple, styled, pdf):
    ...
    p = PlacedTableContent(table, Rect(left=10, top=10, width=280, height=100), pdf)
...
def test_table_with_several_wraps(simple, styled, pdf):
    ...
    p = PlacedTableContent(table, Rect(left=10, top=10, width=180, height=100), pdf)
    assert p.bad_breaks == 0
    assert p.ok_breaks == 3
...
def test_table_with_terrible_wraps(simple, styled, pdf):
    ...
    p = PlacedTableContent(table, Rect(left=10, top=10, width=40, height=100), pdf)
