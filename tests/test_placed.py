...
def test_paragraph_on_one_line(simple, pdf):
    ...
    assert p.error() == PlacementError(surplus_width=27, surplus_height=28, bad_breaks=0, ok_breaks=0)
...
def test_paragraph_which_wraps_once(simple, pdf):
    ...
    p = PlacedFlowableContent(simple, defined, pdf)
    assert p.requested == defined
    assert p.error() == PlacementError(surplus_width=16, surplus_height=16, bad_breaks=0, ok_breaks=1)
...
def test_paragraph_which_wraps_a_lot(simple, pdf):
    ...
    p = PlacedFlowableContent(simple, defined, pdf)
    assert p.requested == defined
    assert p.error() == PlacementError(surplus_width=1, surplus_height=-20, bad_breaks=2, ok_breaks=2)
...
def test_paragraph_which_wraps_badly(simple, pdf):
    ...
    p = PlacedFlowableContent(simple, defined, pdf)
    assert p.requested == defined
    assert p.error() == PlacementError(surplus_width=1, surplus_height=-32, bad_breaks=5, ok_breaks=0)
...
def test_styled_paragraph_on_one_line(styled, pdf):
    ...
    assert p.requested == defined
    assert p.error() == PlacementError(surplus_width=25, surplus_height=20, bad_breaks=0, ok_breaks=0)
...
def test_styled_paragraph_which_wraps_once(styled, pdf):
    ...
    p = PlacedFlowableContent(styled, defined, pdf)
    assert p.requested == defined
    assert p.error() == PlacementError(surplus_width=16, surplus_height=0, bad_breaks=0, ok_breaks=1)
...
def test_styled_paragraph_which_wraps_a_lot(styled, pdf):
    ...
    p = PlacedFlowableContent(styled, defined, pdf)
    assert p.requested == defined
    assert p.error() == PlacementError(surplus_width=1, surplus_height=-60, bad_breaks=2, ok_breaks=1)
...
def test_styled_paragraph_which_wraps_badly(styled, pdf):
    ...
    p = PlacedFlowableContent(styled, defined, pdf)
    assert p.requested == defined
    assert p.error() == PlacementError(surplus_width=1, surplus_height=-80, bad_breaks=3, ok_breaks=0)
...
def test_paragraph_which_wraps_because_of_newline(pdf):
    ...
    p = PlacedFlowableContent(simple, defined, pdf)
    assert p.requested == defined
    assert p.error() == PlacementError(surplus_width=90, surplus_height=16, bad_breaks=0, ok_breaks=0)
...
def test_group(simple, styled, pdf):
    ...
    assert g.error() == PlacementError(surplus_width=36, surplus_height=-20, ok_breaks=1, bad_breaks=3)
...
def test_table_in_plenty_of_space(table, pdf):
    ...
    assert p.error() == PlacementError(surplus_width=100, surplus_height=100 - 20 - 12 - 4 * 5,
...
def test_table_with_one_wraps(table, pdf):
    ...
    assert p.error() == PlacementError(surplus_width=12, surplus_height=100 - 20 - 2 * 12 - 4 * 5,
...
def test_table_with_several_wraps(table, pdf):
    ...
    assert p.error() == PlacementError(surplus_width=3, surplus_height=100 - 2*20 - 2 * 12 - 4 * 5,
...
def test_table_with_terrible_wraps(table):
    ...
    _show(p, pdf)
    assert p.error() == PlacementError(surplus_width=-49, surplus_height=-68,
                                       bad_breaks=0,ok_breaks=3)
